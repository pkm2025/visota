...
from apps.hr.models import Employee, Department, Position, Dependent
...
@pytest.fixture
def company(db):
    return Company.objects.create(code='TCO', name='T', tax_code='0101234567',
...
def make_emp(company):
    def _make(salary=Decimal('15000000'), allowance=Decimal('0')):
        ...
        return Employee.objects.create(
            company=company, code='NV01', full_name='Test',
            department=d, position=p,
            hire_date=date(2020, 1, 1),
            base_salary=salary, allowance=allowance,
        )
    return _make
...
def test_dependent_reduces_pit(company, make_emp):
    ...
    for i in range(2):
        Dependent.objects.create(
            ...
            valid_from=date(2024, 1, 1), registration_status='registered',
        )
...
def test_kpcd_in_voucher(company, make_emp):
    ...
    emp = make_emp(salary=Decimal('15000000'))
    svc = PayrollService(company=company)
    run = svc.calculate('2026-06')
